...
from reasoner.rag.domain.ports import EmbedderPort, FileStorePort, VectorStorePort
from reasoner.rag.parsers import dispatch_parser
    ...
    async def _get_filename(self, file_id: str) -> str:
        async with self._session_factory() as session:
            result = await session.execute(
                text("SELECT filename FROM reasoner.knowledge_files WHERE id = :file_id"),
                {"file_id": file_id},
            )
            row = result.fetchone()
            if row is None:
    ...
    async def _update_status(
        ...
        async with self._session_factory() as session, session.begin():
                await session.execute(
                    text(
                        """
                        UPDATE reasoner.vector_store_files
                        SET status = :status,
