"""RAG NATS handler — subscribes to reasoner.v1.rag.* subjects."""
...
_SUBJECT_INGEST_REQ = "reasoner.v1.rag.ingest.request"
_SUBJECT_INGEST_RES = "reasoner.v1.rag.ingest.result"
_SUBJECT_SEARCH_REQ = "reasoner.v1.rag.search.request"
_SUBJECT_SEARCH_RES = "reasoner.v1.rag.search.result"
_SUBJECT_EMBED_REQ = "reasoner.v1.rag.embed.request"
_SUBJECT_EMBED_RES = "reasoner.v1.rag.embed.result"
_SUBJECT_CHAT_REQ = "reasoner.v1.rag.chat.request"
_SUBJECT_CHAT_RES = "reasoner.v1.rag.chat.result"


class RAGNATSHandler:
    """NATS subscriber for RAG subjects (ingest, search, embed, chat).

    Subscribes to the four reasoner.v1.rag.* request subjects with the
    ...
    async def _handle_ingest(self, msg: Msg) -> None:
        """Handle reasoner.v1.rag.ingest.request → IngestPipeline.ingest."""
    ...
    async def _handle_search(self, msg: Msg) -> None:
        """Handle reasoner.v1.rag.search.request → HybridRetriever.search."""
        start = time.monotonic()
        _log.debug(
            f"nats rag recv: {msg.subject}",
            event_type="message_received",
    ...
    async def _handle_embed(self, msg: Msg) -> None:
        """Handle reasoner.v1.rag.embed.request → EmbedderAdapter.encode."""
        start = time.monotonic()
        _log.debug(
            f"nats rag recv: {msg.subject}",
            event_type="message_received",
    ...
    async def _handle_chat(self, msg: Msg) -> None:
        """Handle reasoner.v1.rag.chat.request → graph with vector_store_ids."""
        start = time.monotonic()
        _log.debug(
            f"nats rag recv: {msg.subject}",
            event_type="message_received",
