...
# arc-friday-migrator is a controlled re-tag of signoz/signoz-schema-migrator.
OTEL_IMAGES := \
...
# Default SigNoz local-dev credentials — override via environment
SIGNOZ_NAME     ?= Admin
SIGNOZ_ORG      ?= ARC
...
otel-up:
	...
	$(COMPOSE_OTEL) up -d
	@printf "$(COLOR_INFO)→$(COLOR_OFF) Waiting for SigNoz to become ready (first run: ~3-5 min)...\n"
	@until curl -sf http://localhost:3301/api/v1/health > /dev/null 2>&1; do \
	  printf "."; sleep 5; \
	done
	@printf "\n$(COLOR_OK)✓$(COLOR_OFF) SigNoz is ready\n"
...
## otel-up-observability: Start the observability backend only (SigNoz + ClickHouse + ZooKeeper)
otel-up-observability:
	...
	@printf "$(COLOR_INFO)→$(COLOR_OFF) Waiting for SigNoz to become ready...\n"
	@until curl -sf http://localhost:3301/api/v1/health > /dev/null 2>&1; do \
	  printf "."; sleep 5; \
	done
	@printf "\n$(COLOR_OK)✓$(COLOR_OFF) SigNoz ready — http://localhost:3301\n"
...
## otel-health: Probe collector (:13133) and SigNoz (:3301) health endpoints
otel-health:
	...
	@printf "$(COLOR_INFO)→$(COLOR_OFF) SigNoz health   (:3301)...   " && \
...
## Override defaults: make otel-user SIGNOZ_EMAIL=you@example.com SIGNOZ_PASSWORD=secret
otel-user:
	@printf "$(COLOR_INFO)→$(COLOR_OFF) Creating SigNoz admin user ($(SIGNOZ_EMAIL))...\n"
	@curl -sf http://localhost:3301/api/v1/register \
	  ...
	  || printf "  User already exists or SigNoz not ready yet (run: make otel-health)\n"
