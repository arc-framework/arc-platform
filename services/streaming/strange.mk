...
STRANGE_IMAGE   := $(REGISTRY)/$(ORG)/arc-streaming
...
## strange-help: Apache Pulsar streaming broker (arc-streaming)
strange-help:
	@printf "\033[1mStrange targets\033[0m\n\n"
	@grep -h "^## strange-" $(MAKEFILE_LIST) | sed 's/^## /  /' | sort
	@printf "\n"

## strange-build: Build arc-streaming image locally using cache (fast)
strange-build:
...
## strange-build-fresh: Build arc-streaming with --no-cache (clean rebuild)
strange-build-fresh: BUILD_FLAGS = --no-cache
strange-build-fresh: strange-build

## strange-up: Start arc-streaming (Pulsar standalone) — waits up to 120s for readiness
strange-up:
...
## strange-down: Stop arc-streaming container; data volume preserved
strange-down:
...
## strange-health: Probe arc-streaming health endpoint (:8082/admin/v2/brokers/health)
strange-health:
...
## strange-logs: Stream arc-streaming container logs
strange-logs:
	$(COMPOSE_STRANGE) logs -f

## strange-clean: [DESTRUCTIVE] Remove arc-streaming container and Pulsar data volume
strange-clean:
...
strange-nuke:
	...
	@printf "$(COLOR_OK)✓$(COLOR_OFF) arc-streaming reset complete — rebuild: make strange-build && make strange-up\n"
## strange-push: Push arc-streaming:latest to ghcr.io (requires: docker login ghcr.io)
strange-push:
...
## strange-publish: Push arc-streaming:latest to ghcr.io (visibility must be set via GitHub UI)
...
## strange-tag: Tag arc-streaming:latest with a version (usage: make strange-tag STRANGE_VERSION=strange-v0.1.0)
strange-tag:
