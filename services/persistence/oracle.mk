...
ORACLE_IMAGE   := $(REGISTRY)/$(ORG)/arc-sql-db
...
## oracle-help: PostgreSQL 17 persistence (arc-sql-db)
oracle-help:
...
## oracle-build: Build arc-sql-db image locally using cache (fast)
oracle-build:
...
## oracle-health: Probe arc-sql-db health (pg_isready :5432)
oracle-health:
...
## oracle-logs: Stream arc-sql-db container logs
oracle-logs:
	$(COMPOSE_ORACLE) logs -f

## oracle-clean: [DESTRUCTIVE] Remove arc-sql-db container and data volume
oracle-clean:
...
oracle-nuke:
	...
	@printf "$(COLOR_OK)✓$(COLOR_OFF) arc-sql-db reset complete — rebuild: make oracle-build && make oracle-up\n"

## oracle-push: Push arc-sql-db:latest to ghcr.io (requires: docker login ghcr.io)
oracle-push:
...
## oracle-publish: Push arc-sql-db:latest to ghcr.io (visibility must be set via GitHub UI)
...
## oracle-tag: Tag arc-sql-db:latest with a version (usage: make oracle-tag ORACLE_VERSION=data-v0.1.0)
oracle-tag:
