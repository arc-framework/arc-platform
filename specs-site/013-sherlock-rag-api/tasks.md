---
url: /arc-platform/specs-site/013-sherlock-rag-api/tasks.md
---
...
* \[x] **\[TASK-001]** \[REASONER] \[P1] Add RAG settings fields to `config.py`
  * Dependencies: none
...
* \[x] **\[TASK-002]** \[PERSISTENCE] \[P1] Add RAG schema SQL init script
...
* \[x] **\[TASK-003]** \[REASONER] \[P1] Add new dependencies to `pyproject.toml`
  * Dependencies: none
...
* \[x] **\[TASK-004]** \[P] \[CI] \[P2] Switch Dockerfile to CPU-only torch + uv (M-1 + M-3)
  * Dependencies: none
...
* \[x] **\[TASK-005]** \[P] \[CI] \[P2] Add pip cache to CI typecheck/test jobs
  * Dependencies: none
...
* \[x] **\[TASK-010]** \[REASONER] \[P1] Create `rag/domain/models.py` with all entity dataclasses
  * Dependencies: TASK-001
...
* \[x] **\[TASK-011]** \[REASONER] \[P1] Create `rag/domain/ports.py` with all port Protocols
  * Dependencies: TASK-010
  * Module: `services/reasoner/src/sherlock/rag/domain/ports.py`
  * Acceptance: Defines `FileStorePort` (upload/download/delete), `VectorStorePort` (init\_schema/upsert\_chunks/search\_hybrid/delete\_by\_file/delete\_vs), `EmbedderPort` (encode), `RerankerPort` (rerank); all use `typing.Protocol` with `@runtime_checkable`; `mypy --strict` passes
...
* \[x] **\[TASK-020]** \[P] \[REASONER] \[P1] Implement `rag/adapters/minio.py` (MinIO file store)
...
* \[x] **\[TASK-021]** \[P] \[REASONER] \[P1] Implement `rag/adapters/pgvector.py` (vector + FTS store)
...
* \[x] **\[TASK-022]** \[P] \[REASONER] \[P1] Implement `rag/adapters/embedder.py` (SentenceTransformer wrapper)
  * Dependencies: TASK-010, TASK-011
...
* \[x] **\[TASK-023]** \[P] \[REASONER] \[P1] Implement `rag/adapters/reranker.py` (CrossEncoder wrapper)
  * Dependencies: TASK-010, TASK-011
...
* \[x] **\[TASK-030]** \[P] \[REASONER] \[P1] Implement `rag/parsers/` (all format parsers)
  * Dependencies: TASK-010
...
* \[x] **\[TASK-031]** \[P] \[REASONER] \[P1] Implement `rag/chunker.py` (token-aware text chunker)
  * Dependencies: TASK-010
...
* \[x] **\[TASK-040]** \[REASONER] \[P1] Implement `rag/application/ingest.py` (IngestPipeline)
...
* \[x] **\[TASK-041]** \[REASONER] \[P1] Implement `rag/application/retrieve.py` (HybridRetriever)
...
* \[x] **\[TASK-042]** \[REASONER] \[P1] Implement `rag/store.py` (RAGInfra factory + dataclass)
...
* \[x] **\[TASK-050]** \[P] \[REASONER] \[P1] Implement `files_router.py` (US-1, US-6, FR-1/2/3)
...
* \[x] **\[TASK-053]** \[P] \[REASONER] \[P1] Extend `graph.py` with RAG retrieve node (FR-9)
...
* \[x] **\[TASK-054]** \[P] \[REASONER] \[P1] Wire tiktoken token counts + file\_search tool detection in `openai_router.py` (FR-9, FR-14, US-13)
...
* \[x] **\[TASK-055]** \[P] \[REASONER] \[P2] Implement `rag/nats_handler.py` (RAG NATS subjects, US-12)
  * Dependencies: TASK-042
...
* \[x] **\[TASK-060]** \[REASONER] \[P1] Wire `RAGInfra` into `main.py` (AppState + lifespan + router mounts)
...
* \[x] **\[TASK-061]** \[P] \[REASONER] \[P1] Update `docker-compose.yml` with RAG env vars
  * Dependencies: TASK-001
...
* \[x] **\[TASK-062]** \[P] \[REASONER] \[P1] Bump `service.yaml` to v0.3.0, add `storage` depends\_on
  * Dependencies: TASK-001
...
* \[x] **\[TASK-070]** \[P] \[REASONER] \[P1] Write unit tests for `rag/` package
...
* \[x] **\[TASK-071]** \[P] \[REASONER] \[P1] Write integration tests for all 3 new routers + RAG chat
  * Dependencies: TASK-060
