---
url: /arc-platform/specs-site/013-sherlock-rag-api/analysis-report.md
---
...
|-----------|----------|----------------------|---------|--------|
| EC-1 | Unsupported extension | 400 error | TASK-050 (files\_router) | ✓ |
| EC-2 | File exceeds max size (50 MB) | 400 error | TASK-050 (files\_router) | ✓ |
| EC-3 | Ingest fails (corrupt PDF, parser exception) | `status=failed`, error\_message in DB | TASK-040 (IngestPipeline exception handling) | ✓ |
...
| EC-6 | Non-existent vector\_store\_id | Empty results, rest still searched | TASK-041 (HybridRetriever) | ✓ |
| EC-7 | Same file added to VS twice | Idempotent (returns existing row) | TASK-051 (vector\_stores\_router) | ✓ |
...
| EC-10 | `?sync=true` on file >30s | Timeout → fallback to 202 | TASK-051 (asyncio.wait\_for with timeout) | ✓ |
...
4. **Parallel Batch C — HTTP Routers (6 tasks, 6 parallel after TASK-042):**
   * TASK-050 (files\_router)
   * TASK-051 (vector\_stores\_router)
   * TASK-052 (embeddings\_router)
   * TASK-053 (graph.py RAG node)
   * TASK-054 (openai\_router tiktoken)
...
* **Issue:** Plan.md says "/health/deep includes `minio: bool` component when `rag_enabled`" but existing health\_deep() (main.py:283) only checks postgres + nats.
...
* **File:** TASK-023 (reranker\_router)
...
* Current config.py and openai\_router already import tiktoken, but 012 returns `(0, 0, 0)`. TASK-054 must wire this.
...
* Consider: What happens if the same file is re-added before the first ingest completes?
  ...
  * Recommendation: TASK-051 should clarify: insert vector\_store\_files with unique constraint (vs\_id, file\_id) to prevent duplicates.
...
4. **TASK-060 (main.py wiring)**: Document when RAGInfra is None (rag\_enabled=false) — all routers must return 503 immediately. Confirm check happens before any logic in routers.
...
| File deletion during in-flight ingest | M | FK cascade deletes chunks; `vector_store_files` status=failed with error\_message | TASK-040, TASK-051 |
