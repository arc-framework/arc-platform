---
url: /arc-platform/specs-site/012-sherlock-chat-api/plan.md
---
...
|--------|----------|---------|
| `services/reasoner/` | Python 3.13 | New files: providers package, models\_v1, interfaces, streaming, two routers, NATS v1 handler. Modified: main, config, llm\_factory, observability, contracts, pyproject, Dockerfile |
...
| C | `streaming.py` + `models_router.py` | Yes — both only need models\_v1 + interfaces |
| D | `openai_router.py` + `openai_nats_handler.py` | Yes — both call invoke\_graph / stream\_graph |
