---
url: /arc-platform/specs-site/015-nervous-system/analysis-report.md
---
...
|-----|-------------|-----------|--------------|---------|--------|
| FR-1 | stream\_graph() default | Functional | Phase 1 | T010, T011 | MAPPED |
...
| NFR-5 | stream\_graph() non-blocking | Non-Functional | Phase 1 | T010, T011, T019 | MAPPED |
...
* T032 acceptance: "SHERLOCK\_GUARD\_ENABLED=false default disables hooks (fail-open)"
...
**Recommendation**: Documented and mitigated. No change needed. Ensure T032 includes test\_guard\_pre\_check\_rejects\_injection and test\_guard\_post\_check\_intercepts\_unsafe\_output as **mock/stub tests** until RoboCop exists.
...
**Issue**: Acceptance criterion says "test\_graph.py parallel retrieve test passes" but does not specify **what the test must verify** (e.g., context injection timestamp, state update event, LLM sees full context before N-th token).
...
### RISK-1: stream\_graph() API Stability
...
**Recommendation**: Before T010 starts, add a pre-flight analysis task to audit LangGraph release notes for stream\_graph() error behavior. Ensure acceptance criteria for T010/T011 include: **"Test: LLM API timeout mid-stream → error chunk published to NATS + completion signal sent"**
...
**Description**: Spec.md and plan.md suggest asyncapi.yaml should be generated from Pydantic models in models\_v1.py to prevent schema drift, but tasks.md T035 does not include automation.

**Evidence**:

* Plan.md Risk #7: "asyncapi.yaml schema drift from actual event payloads"
* Mitigation: "Generate schema from Pydantic models in models\_v1.py; keep them the single source of truth"
...
3. Documentation comment in models\_v1.py: "Source of truth for all event schemas — update asyncapi.yaml when these models change"
...
| VI | Local-First | YES | **PASS** | Pulsar degrades gracefully (SHERLOCK\_PULSAR\_ENABLED flag); NATS path unaffected if Pulsar unavailable |
...
| XI | Resilience | YES | **PASS** | Pulsar fail-open (SHERLOCK\_PULSAR\_ENABLED=false); Redis fail-open; RoboCop fail-open; NATS/Pulsar fallback |
...
**TASK-010 \[P] Wire stream\_graph() into nats\_handler.py**
...
**Phase 1 Checklist**: 5 items (invoke\_graph removal, token publishing, executor usage, ttft metric, lint/test)
...
3. **RISK-1**: stream\_graph() error handling not pre-flighted
...
6. Feature flags (SHERLOCK\_PULSAR\_ENABLED, SHERLOCK\_GUARD\_ENABLED) documented but naming should be standardized in config.py
...
1. **Pre-flight stream\_graph() analysis**: Audit LangGraph release notes for error handling differences. Add error path tests to T010/T011 acceptance criteria.
