---
url: /arc-platform/specs-site/011-vector-setup/tasks.md
---
...
* \[x] **\[TASK-023]** \[P] \[REASONER] \[P1] Add full OTEL env vars + Docker healthcheck to Sherlock docker-compose
  ...
  * Acceptance: All 9 `OTEL_*` env vars present; `OTEL_EXPORTER_OTLP_ENDPOINT` value is `http://arc-friday-collector:4317` (service DNS); `healthcheck:` block added (`wget -qO- http://localhost:8000/health`, interval=15s, retries=5, start\_period=60s); `SHERLOCK_QDRANT_HOST` and `SHERLOCK_QDRANT_PORT` removed
...
* \[x] **\[TASK-030]** \[P] \[REASONER] \[P1] Rewrite `test_memory.py` for pgvector
  ...
  * Acceptance:
    ...
    * Test coverage: `init()` idempotent (called twice — no error), `search()` returns list of strings with user\_id filter, `save()` inserts embedding, `health_check()` returns `{"postgres": bool}`
...
* \[x] **\[TASK-040]** \[PLATFORM] \[P1] Wire profiles + add persistence healthcheck + verify end-to-end
  ...
  * Acceptance:
    ...
    * `services/persistence/docker-compose.yml` has `healthcheck:` block (`pg_isready -U arc`, interval=10s, retries=5, start\_period=30s)
