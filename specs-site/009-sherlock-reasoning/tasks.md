---
url: /arc-platform/specs-site/009-sherlock-reasoning/tasks.md
---
...
* \[x] **\[TASK-010]** `[SERVICES]` `[P1]` Implement `config.py` — Pydantic BaseSettings foundation
  ...
  * Acceptance:
    ...
    * All `SHERLOCK_*` env var aliases present for: service\_name, version, postgres\_url, qdrant\_host, qdrant\_port, qdrant\_collection, nats\_url, nats\_enabled, nats\_subject, nats\_queue\_group, pulsar\_url, pulsar\_enabled (default `False`), pulsar\_request\_topic, pulsar\_result\_topic, pulsar\_subscription, llm\_model, llm\_base\_url, embedding\_model, embedding\_dim, context\_top\_k, otel\_endpoint, otel\_traces\_enabled, otel\_metrics\_enabled, content\_tracing (default `False`)
...
* \[x] **\[TASK-032]** `[P]` `[SERVICES]` `[P2]` Implement `pulsar_handler.py` — durable async Pulsar consumer
  ...
  * Acceptance:
    ...
    * **\[GAP-6 resolved]** `_process(msg)` failure path — two distinct outcomes:
      * **Path A — graph returned error string** (error\_handler capped retries): `invoke_graph` returns a string; `_process` publishes `{"request_id":..., "error": str, "latency_ms": N}` to `sherlock-results` → **`consumer.acknowledge(msg)`** (successful processing of bad input)
...
* \[x] **\[TASK-054]** `[P]` `[SERVICES]` `[P2]` Write `tests/test_pulsar_handler.py` — PulsarHandler tests
  ...
  * Acceptance:
    ...
    * **\[GAP-6 resolved]** `test_error_handler_result_published_and_acked`: `invoke_graph` returns error string (simulating error\_handler exhausted retries) → `{"request_id":..., "error": str, "latency_ms": N}` published to `sherlock-results` → `consumer.acknowledge(msg)` called — NOT negative-acked (it was successfully processed)
...
| After T030 | T031, T032 in parallel | invoke\_graph exists |
