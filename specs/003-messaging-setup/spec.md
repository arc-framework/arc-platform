...
- [ ] FR-1: Create `services/messaging/` with a NATS Dockerfile pinning `nats:2.10-alpine`, `service.yaml`, `docker-compose.yml`, and `flash.mk`
- [ ] FR-2: Create `services/streaming/` with a Pulsar Dockerfile pinning `apachepulsar/pulsar:3.3.0`, `service.yaml`, `docker-compose.yml`, and `strange.mk`
- [ ] FR-3: Create `services/cache/` with a Redis Dockerfile pinning `redis:7.4-alpine`, `service.yaml`, `docker-compose.yml`, and `sonic.mk`
