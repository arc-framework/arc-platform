...
- TASK-030 (test_memory.py rewrite) doesn't mention protocol compliance testing
...
- TASK-020 acceptance criteria state the hook must be "tested in test_memory.py"
...
- Line 69 (TASK-022): "All 9 OTEL_* env vars present" — doesn't specify endpoint value
...
**Issue**: Risk mitigation states "start_period: 60s in healthcheck gives model time to load". However:
...
- TASK-030 (test_memory.py) doesn't mention verifying `init()` can be called twice without error
...
|---------|---------|--------|----------|
| Factory/DI | REQUIRED (Python) | PASS | `SherlockMemory` accepts engine + logger via __init__; no globals |
| Repository | REQUIRED (Python) | PASS | `MemoryBackend` Protocol defined; `SherlockMemory` implements domain-specific methods (search, save, health_check) |
| Config Precedence | REQUIRED (Python) | PASS | Config cleanup removes hard-coded qdrant_* settings; OTEL via 12-Factor env vars |
