...
- [x] [TASK-021] [P] [SERVICES] [P1] Implement `internal/clients/nats.go` — JetStream stream creation + circuit breaker
  ...
  - Note: Stream names use SCREAMING_SNAKE_CASE convention (`AGENT_COMMANDS`, `AGENT_EVENTS`, `SYSTEM_METRICS`) instead of the original spec placeholder names (`arc-commands`, `arc-events`, `arc-results`). NATS JetStream streams are conventionally uppercased; this is an intentional naming improvement.
